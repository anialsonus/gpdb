...
```bash
docker build -t gpdb7_regress:latest -f arenadata/Dockerfile . --build-arg CC=clang --build-arg CXX=clang++ --build-arg USE_CLANG_ANALYZER=true --build-arg SKIP_UNITTESTS=true
```
